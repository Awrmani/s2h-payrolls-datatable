...
class LivewireDatatable extends Component
{
    ...
    public function mount(
        ...
        foreach (['model', 'include', 'exclude', 'hide', 'dates', 'times', 'searchable', 'sort', 'hideHeader', 'hidePagination', 'perPage', 'exportable','hideable', 'actions', 'beforeTableSlot', 'afterTableSlot', 'addtionalSearch', 'customizeCreateForm', 'additionalButtons'] as $property) {
    ...
    public function getViewColumns()
    {
        return collect($this->freshColumns)->map(function ($column) {
            $columns = ['hidden', 'label', 'align', 'type', 'input', 'filterable', 'filterview', 'name', 'class', 'events', 'input_class'];
    ...
    {
        $output = substr($value, stripos($value, (string) $string), strlen($string));
